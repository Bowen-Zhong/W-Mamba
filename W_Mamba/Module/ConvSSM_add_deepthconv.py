...
class SS2D(
    nn.Module):  # SS2D（State Space adafuse 2D）类，它是实现状态空间模型（SSM）在视觉任务中应用的核心部分。这个类将状态空间模型的长程依赖性建模能力与卷积操作结合起来，旨在捕获图像中的长程相互作用，同时保持对局部特征的敏感度。
    def __init__(
            self,
            d_model,
            d_state=16,
...
if __name__ == '__main__':
    ...
    block = ConvSSM(hidden_dim=256).to(device)
