...
def evaluation_one(ir_name, vi_name, f_name):
    ...
    return EN, SF, AG, SD, CC, SCD, VIF, MSE, PSNR, MI, SSIM, MS_SSIM, Qabf
...
if __name__ == '__main__':
    ...
    Fuse_path = "../result/ASFE/PET-MRI"
    result_path = '../assessment_indexes/ASFE/PET-MRI'
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    IR_image_list = os.listdir(IR_path)
    VIS_image_list = os.listdir(VIS_path)
    Fuse_imag_list = os.listdir(Fuse_path)
    ...
    Fuse_image_list = sorted(Fuse_imag_list, key=lambda i: int(re.search(r'(\d+)', i).group()))
    evaluation_results = []
    num = 0
    ...
    # 将结果保存到CSV文件
    csv_file_path = os.path.join(result_path, 'evaluation_results_{}.csv'.format(model_name))
    write_results_to_csv(evaluation_results, csv_file_path)

    print(f"评估结果已保存到CSV文件：{csv_file_path}")
# 完成循环后，evaluation_results 列表将包含所有图像对的评估结果
