...
from eval_one_image import write_results_to_csv, evaluation_one
#from network_add_WTUNETconvSSMdeepcnv_FATloss import ASFEFusion
# from network_add_WTUNETconvSSMdeepcnv_FATloss import ASFEFusion
...
def Mytest(model_test=None, img_save_dir=None):
    ...
    if model_test is None:
        ...
        model_path_final = './modelsave/' + args.model + '/' + args.task + '/' + natsorted(os.listdir(model_dir))[0]
        #model_path_final = '/home/user/cy/ASFE-Fusion-main/modelsave/our_FATloss_wtlevel_truth2_10_1000_10_100_rotated/PET-MRI_rotated/3_ASFEFusion.pth'
    ...
    with torch.no_grad():
        if args.task == 'CT-MRI':
            ...
            for batch, [img_name, img1_Y, img2, img1_CrCb] in enumerate(test_loader):  # PET/SPECT-MRI_rotated-MRI Fusion
                print("test for image %s" % img_name[0])
    ...
    IR_path = "./HMIFDatasets/PET-MRI/test/PET"
    VIS_path = "./HMIFDatasets/PET-MRI/test/MRI"
    ...
    # 将结果保存到CSV文件
    csv_file_path = os.path.join(result_path, 'PETevaluation_results_{}_{}.csv'.format(model_name, args.epoch))
...
if __name__ == '__main__':
    '''Mytest(
        model_test="./modelsave/CT-MRI/2000_ASFEFusion.pth",
